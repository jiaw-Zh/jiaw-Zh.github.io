...
这里的坑就更多了，厂商删掉了超管账号就算了，用户手动添加的每次重启后还给你删掉了

我试过了修改 `/etc/rc.d/rc.local` 文件，也试过编写 `crontab` 脚本，都没有生效，我以为是功能不行，现在想来都是因为那个开机脚本。

上面的两个方法谷歌的关键字是：Linux系统开机自动运行脚本

后来改变思路，搜索：openwrt开机自动运行脚本

还是有一点变化的，openwrt需要编写的脚本在 `/etc/init.d/` 目录下

然后开始按照规则编写这里的脚本，其实本来我是可以成功的，但是厂商自己写的脚本优先级很低，每次都在我的运行之后再运行，如果我把优先级写的再低些就没事了，
不过还好没成功，不然也不会发现 FOTA 的真实开关

到这里我已经猜到重置超管密码的脚本就写在这个目录下的某个文件里了

然后执行这个命令，在目录下所有文件中查找内容

`find /etc/init.d/ -name '*'| xargs grep 'mdlcfg'`

一下子找到了删除语句，然后就是修改 `/etc/init.d/tz_process_start` 文件
